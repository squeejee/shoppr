...
class ClientTest < Test::Unit::TestCase
  ...
  context "When mapping generic search response XML to ruby objects" do
    setup { @generic_response = GenericSearchResponse.from_xml(fixture_file('general_search_specs_offers_reviews.xml')) }
    
    should "include server detail information" do
      @generic_response.server_detail.api_version.should == '3.1 R21.4'
      @generic_response.server_detail.api_env.should == 'sandbox'
    end
    
    should "include exceptions" do
      @generic_response.exceptions.size.should == 1
      @generic_response.exceptions.first.code.should == 1112
    end
    
    should "include client tracking info" do
      @generic_response.client_tracking.height.should == 19
      @generic_response.client_tracking.width.should == 106
      @generic_response.client_tracking.type.should == 'logo'
      @generic_response.client_tracking.alt_text.should == 'Digital Cameras'
    end
    
    should "include search history info" do
      @generic_response.search_history.category_selections.size.should == 3
      @generic_response.search_history.category_selections.last.id.should == 7185
      @generic_response.search_history.category_selections.last.category_url.should == 'http://www.shopping.com/xPP-digital_cameras-nikon~linkin_id-7000610?oq=nikon'
    end
    
    should "include related terms" do
      @generic_response.related_terms.size.should == 20
      @generic_response.related_terms.last.should == 'canon powershot sd800'
    end
    
    should "include categories" do
      @generic_response.categories.size.should == 1
    end
    
    context "when mapping categories" do
      setup do
        @categories = @generic_response.categories
      end
